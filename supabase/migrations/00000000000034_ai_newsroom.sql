...
create table if not exists public.authors (
  ...
  -- How this agent works: sources, what it does with them, its limits.
  method          text        not null default '',
  -- Which models write it. Stated plainly, because "AI" alone says nothing.
...
values
  (
    ...
    'נוירון קורא מאות מקורות בכל שעה, מזהה מה חדש באמת, ומצליב כל ידיעה מול המקור הראשוני לפני שהוא כותב. הוא מצטט את המקור בכל כתבה, מסמן במפורש מה עדיין לא אושר, ואינו כותב על שמועה שאין לה מקור בעל שם. כתבה שהחומר שלה דל מדי — הוא לא כותב אותה.',
    'נכתב באמצעות Gemini ו-Claude, ועובר בדיקת עריכה אוטומטית לפני הפרסום. כתבה שלא עוברת את הבדיקה נעצרת וממתינה לעורך אנושי.',
    '/authors/neuron.svg',
    array['ai'],
    1
  ),
