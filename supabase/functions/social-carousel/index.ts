...
//
// Why the stages: the script takes a model call, each background is an image
// generation, and a slide render is heavy enough that two in one worker exhaust
// it. So `create` returns once the script is written, and the panel calls
// `tick` every few seconds; each tick does ONE heavy thing — generate the next
// backgrounds, or render the next slide — until every slide has its PNG. A
// closed panel just leaves the carousel paused; opening it again resumes it.
//
// Backgrounds come from Gemini (already connected) unless
// CAROUSEL_IMAGE_PROVIDER=higgsfield. Words are never drawn by the image
// model; the Hebrew is typeset here, because image models misspell it.
...
type Slide = {
  ...
  /** Waiting for a tick to generate its background synchronously (Gemini). */
  bg_pending?: boolean;
  bg_url: string | null;
...
type Provider = "gemini" | "higgsfield";
const provider = (): Provider =>
  (Deno.env.get("CAROUSEL_IMAGE_PROVIDER") || "").toLowerCase() === "higgsfield" && higgsfieldConfigured()
    ? "higgsfield"
    : "gemini";
const providerReady = async () => (provider() === "higgsfield" ? true : !!(await getSecret("GEMINI_API_KEY")));

/**
 * How many Gemini backgrounds one tick generates. Two in parallel halves the
 * wait without leaning on the free tier's per-minute limit; the images are
 * decoded and stored, never rendered, in the same worker.
 */
const GEMINI_PER_TICK = 2;
const MIN_SLIDES = 5;
const MAX_SLIDES = 8;
...
async function startBackground(slide: Slide, articleImage: string): Promise<Slide> {
  if (provider() === "gemini") {
    if (!(await getSecret("GEMINI_API_KEY"))) {
      return { ...slide, hf_request_id: null, bg_pending: false, bg_url: articleImage, bg_source: "article", bg_error: "GEMINI_API_KEY חסר" };
    }
    // Generated by a later tick: Gemini answers synchronously, and eight of those
    // inside the create call would outlast it.
    return { ...slide, hf_request_id: null, bg_pending: true, bg_url: null, bg_source: null, bg_error: null };
  }
  try {
    const requestId = await submitImage(`${slide.prompt}${VISUAL_SUFFIX}`);
    return { ...slide, hf_request_id: requestId, bg_pending: false, bg_url: null, bg_source: null, bg_error: null };
  } catch (e) {
    return { ...slide, hf_request_id: null, bg_pending: false, bg_url: articleImage, bg_source: "article", bg_error: (e as Error).message };
  }
}
...
async function tick(supabase: any, id: string): Promise<Carousel> {
  ...
  // 1. Collect whatever Higgsfield has finished (when it is the provider).
  await Promise.all(
  ...
  // 2. Gemini backgrounds still owed? Generate the next few and stop there:
  //    image work and a render never share a worker.
  const owed = slides
    .map((s, i) => ({ s, i }))
    .filter(({ s }) => s.bg_pending && !s.bg_url)
    .slice(0, GEMINI_PER_TICK);
  if (owed.length > 0) {
    await Promise.all(
      owed.map(async ({ s: slide, i }) => {
        try {
          const url = await generateGeminiImage(supabase, `${slide.prompt}${VISUAL_SUFFIX}`, {
            aspectRatio: "4:5",
            pathPrefix: `social/carousels/${carousel.id}/bg-${i}-v${slide.version}`,
          });
          slides[i] = { ...slide, bg_pending: false, bg_url: url, bg_source: "gemini", bg_error: null };
        } catch (e) {
          // The carousel must not stall on one image: the slide takes the
          // article photo and says why, and "רקע חדש" can try again later.
          slides[i] = {
            ...slide,
            bg_pending: false,
            bg_url: article.image_url,
            bg_source: "article",
            bg_error: (e as Error).message,
          };
        }
      }),
    );
    carousel = await saveCarousel(supabase, carousel, { slides, status: "generating" });
    return carousel;
  }

  // 3. Render one slide — one per worker, the renderer's hard limit.
  ...
  try {
    ...
    switch (body?.action) {
      case "create": {
        ...
        return json({
          carousel: await create(supabase, articleId),
          provider: provider(),
          providerReady: await providerReady(),
        });
      }
      case "tick":
