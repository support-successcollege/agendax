...
const provider = (): Provider =>
  (Deno.env.get("CAROUSEL_IMAGE_PROVIDER") || "").toLowerCase() === "higgsfield" && higgsfieldConfigured()
    ? "higgsfield"
    : "gemini";
const providerReady = () => (provider() === "higgsfield" ? true : !!Deno.env.get("GEMINI_API_KEY"));
  ...
  try {
    ...
    switch (body?.action) {
      case "create": {
        ...
        return json({ carousel: await create(supabase, articleId), provider: provider(), providerReady: providerReady() });
      }
      case "tick":
