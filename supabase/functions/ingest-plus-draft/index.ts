// Public Edge Function for ingesting AI-generated articles from News Creator Plus
// Auth: shared secret in `x-plus-secret` header. Creates an article as draft.
import { createClient } from "npm:@supabase/supabase-js@2";

const corsHeaders = {
  "Access-Control-Allow-Origin": "*",
  "Access-Control-Allow-Headers":
    "authorization, x-client-info, apikey, content-type, x-plus-secret",
...
Deno.serve(async (req) => {
  ...
  const expected = Deno.env.get("PLUS_INGEST_SECRET");
  if (!expected) return json(500, { error: "Server not configured" });

  const provided = req.headers.get("x-plus-secret");
  ...
  const bgDataUrl = typeof body?.bgDataUrl === "string" ? body.bgDataUrl : "";
  const author = typeof body?.author === "string" && body.author.trim() ? body.author.trim() : "מחולל AI";

  if (!headline || !article) return json(400, { error: "Missing headline or article" });
