...
קריטריונים לבחירה, לפי סדר חשיבות (הסדר הזה צריך להשתקף גם ב-priority):
1. השלכה ישירה על עובד הייטק או משקיע ישראלי: משרות, פיטורים, גיוסי עובדים, שכר, כישורים שנדרשים, מניות שישראלים מחזיקים, השפעת AI על הקריירה. ידיעה כזו עולה על דיווח מוצר גנרי, גם אם המוצר מפורסם יותר.
2. חברה או דמות ישראלית, או אירוע שמתרחש בישראל: בונוס ברור. סטארטאפים, מרכזי פיתוח, יזמים ומשקיעים ישראלים, רגולציה ישראלית.
3. דרמה עסקית: סכסוך, תביעה, מיזוג/רכישה, פיטורים, גיוס גדול, קריסה או משבר בחברה. סיפור עם צדדים ומחלוקת מנצח הודעה יבשה.
4. הודעות מוצר ומודלים של ענקיות בלי זווית ישראלית: רק אם הן גדולות באמת (מודל מדור חדש, מוצר שפותח קטגוריה). עדכון גרסה, פיצ'ר או שיפור הדרגתי לא עוברים.
- עניין לקורא: משהו שקורא הייטק ישראלי ירצה לדעת עליו הבוקר, ושאפשר לענות עליו "מה זה אומר לי".
...
- angle: משפט אחד בעברית — הזווית שבה כדאי לכתוב את הכתבה לקורא הישראלי. עדיף זווית ישימה: מה זה אומר לעובד, למשקיע או לחברה בישראל.
